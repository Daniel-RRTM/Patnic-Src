...
func _ready():
	var stationEnum = Davinci.externalCachedInput
	API_003_Player.stations.changeStation(stationEnum)
...
func loadIcons() -> void:
	for station in ENUM.SOKRATILES.M_TYPE.keys():
		get_node("Panel/Station title/"+station).visible = ENUM.SOKRATILES.M_TYPE[station] == Davinci.externalCachedInput
