...
static func getIndexByMode(mode:String,compName:String) -> String:
	match mode:
		...
		"Connection"         : return API_001_Atlas.Connection().getEntry(compName).name()
		"Quest"         : return API_001_Atlas.Quest().getEntry(compName).name()
		"LootFilter"         : return API_001_Atlas.LootFilter().getEntry(compName).getCompValue("C_6_NAME")
...
static func getObjectByMode(mode:String,compName:String):
	match mode:
		...
		"Connections"         : return API_001_Atlas.Connection().getEntry(compName)
		"Quests"         : return API_001_Atlas.Quest().getEntry(compName)
		"LootFilter"         : return API_001_Atlas.LootFilter().getEntry(compName)
...
static func runParserByMode(mode:String,compName,filepath:String):
	match mode:
		...
		"Connections"         : Statistics_Parser_Generall.calc(mode,filepath,compName)
		"Quests"         : Statistics_Parser_Generall.calc(mode,filepath,compName)
		"LootFilter"         : Statistics_Parser_Generall.calc(mode,filepath,compName)
